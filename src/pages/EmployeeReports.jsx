...
const EmployeeReports = () => {
  ...
  // Fetch Sales Users List (Branch Panel filters to only branch users on backend automatically)
  useEffect(() => {
    const fetchSalesUsers = async () => {
      try {
        ...
        toast.error("Failed to load branch sales employees list");
      }
    };
    if (token) {
      fetchSalesUsers();
    ...
    <div className="p-6 animate-fade-in max-w-7xl mx-auto">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-bold tracking-tight" style={{ color: themeColors.text }}>
              Branch Employee Reports
            </h1>
          </div>
          <p className="text-sm" style={{ color: themeColors.textSecondary }}>
            Detailed performance analytics, deal values, and metrics for your branch sales executives.
          </p>
        </div>
      </div>
      ...
      <div className="mb-6 p-6 rounded-xl border shadow-sm" style={{ backgroundColor: themeColors.surface, borderColor: themeColors.border }}>
        <div className="flex flex-col md:flex-row items-end gap-4">
          <div className="flex-1 w-full">
            ...
            >
              <option value="">-- Choose Branch Sales Executive --</option>
              {salesUsers.map((u) => (
      ...
      {isFetching ? (
        <div className="py-20 flex flex-col items-center justify-center">
          ...
          <p className="mt-4 font-medium" style={{ color: themeColors.textSecondary }}>Generating branch employee reports...</p>
        </div>
      ) : currentData ? (
        <>
          {/* Lead KPI Cards */}
        ...
        <div className="text-center py-20 rounded-xl border border-dashed" style={{ borderColor: themeColors.border }}>
          <p className="text-lg font-medium" style={{ color: themeColors.textSecondary }}>Select a branch sales employee to view their detailed performance report.</p>
        </div>
      )}
